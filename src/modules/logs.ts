...
export let findByModule = function (
  ...
  LoggingModel.find({ module: module }, function (err:any, result:any) {
    cb(err, result);
  })
    .sort({ createdAt: -1 })
    .limit(limit);
...
export let findByLogLevel = function (
  ...
  LoggingModel.find({ level: level }, function (err:any, result:any) {
    cb(err, result);
  })
    .sort({ createdAt: -1 })
    .limit(limit);
...
export let findByTransactionId = function (txId: string, cb: Function) {
  LoggingModel.find({ transactionId: txId }, function (err:any, result:any) {
    cb(err, result);
  }).sort({ createdAt: -1 });
};
